...
6. Lead Cards with plan status, progress, next recommendation and the bill checklist. Show an explained value rank beside the proposal, with nearby comparisons available there. Rank distinct currently evidenced offers by conservative net value; disclose catalog coverage and keep unverified figures unranked. Show alternatives up to 20% above calculated spending capacity, including the seven-day buffer and exact total/monthly shortfall. A higher-spend proposal requires a saved description of already-planned purchases and an additional approval confirmation; it does not inflate the calculated baseline or forecast. Keep supporting wallet/history tools below. Reuse top-bar Actions for timely follow-ups; Today must never render action cards.
