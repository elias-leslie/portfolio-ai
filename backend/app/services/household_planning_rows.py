...
def row_to_housing_cost(
    ...
    *,
    ...
    return HouseholdHousingCost(
        ...
        property_address=str(row[4]) if row[4] is not None else None,
        monthly_payment=to_float(row[5]),
        property_tax_monthly=to_float(row[6]),
        hoa_monthly=to_float(row[7]),
        insurance_monthly=to_float(row[8]),
        utilities_monthly=to_float(row[9]),
        maintenance_monthly=to_float(row[10]),
        mortgage_balance=to_float(row[11]),
        interest_rate=to_float(row[12]),
        property_value=to_float(row[13]),
        ownership_percent=to_float(row[14]),
        value_as_of=iso_or_none(row[15]),
        valuation_source=str(row[16]) if row[16] is not None else None,
        valuation_confidence=to_float(row[17]),
        valuation_range_low=to_float(row[18]),
        valuation_range_high=to_float(row[19]),
        retirement_treatment=str(row[20] or "track_only"),
        annual_retirement_income=to_float(row[21]),
        liquidity_year=int(row[22]) if row[22] is not None else None,
        liquidity_amount=to_float(row[23]),
        notes=str(row[24]) if row[24] is not None else None,
        confirmation_status=str(row[25]),
        provenance=str(row[26]),
        evidence_note=str(row[27]) if row[27] is not None else None,
        source_document_id=str(row[28]) if row[28] is not None else None,
        created_at=iso(row[29]),
        updated_at=iso(row[30]),
    )
...
def row_to_retirement_healthcare_schedule(
    row: tuple[Any, ...],
    *,
    iso: Callable[[Any], str],
    to_float: Callable[[Any], float | None],
