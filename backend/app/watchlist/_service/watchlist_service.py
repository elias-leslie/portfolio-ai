...
class WatchlistService:
    ...
    def get_items_with_scores(self, *, include_decision: bool = True) -> list[dict[str, Any]]:
        ...
        item_ids = [str(row["id"]) for row in items_df.iter_rows(named=True)]
        quote_map = build_quote_map(self.storage, symbols)
        price_trend_map = build_price_trend_map(self.storage, symbols)
        score_series_map = build_score_series_map(self.storage, item_ids)
        ...
        for row in items_df.iter_rows(named=True):
            ...
            item_data["score_trend"] = score_series_map.get(str(row["id"]))
    ...
    def get_item_with_score_by_id(self, item_id: str) -> dict[str, Any] | None:
        ...
        item_data["quote"] = quote_map.get(str(symbol).upper())
        item_data["price_trends"] = build_price_trend_map(self.storage, [symbol]).get(
            str(symbol).upper(), []
        )
        item_data["score_trend"] = build_score_series_map(self.storage, [str(row["id"])]).get(
            str(row["id"])
        )
