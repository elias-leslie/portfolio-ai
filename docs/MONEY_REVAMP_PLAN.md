...
1. **2.4 — new this month** (D2.4): novelty detection on merchants with no prior
   history, clustered where they belong (the July El Salvador trip = 8
   merchants, $385, one cluster — not 8 mystery lines). Needs owner attribution
   to be worth anything for D2's fourth sentence, and owner is still 91%
   "Family".
2. **2.5 — affordability panel** (1.4), then 2.6 and 2.7.
