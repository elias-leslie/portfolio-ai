...
def row_to_profile(
    ...
    *,
    ...
    return HouseholdProfile(
        ...
        filing_status=str(row[18]) if row[18] is not None else None,
        state_of_residence=str(row[19]) if row[19] is not None else None,
        effective_tax_rate=to_float(row[20]),
        marginal_federal_tax_rate=to_float(row[21]),
        marginal_state_tax_rate=to_float(row[22]),
        emergency_fund_target_months=to_float(row[23]),
        emergency_fund_target_amount=to_float(row[24]),
        notes=str(row[25]) if row[25] is not None else None,
        created_at=iso(row[26]),
        updated_at=iso(row[27]),
    )
