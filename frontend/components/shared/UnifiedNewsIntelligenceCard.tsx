...
export function UnifiedNewsIntelligenceCard({
  ...
  const { balancedArticles, positiveCount, negativeCount } = useMemo(() => {
    if (sortBy !== 'recent') {
      ...
      return {
        balancedArticles: sortedArticles,
        positiveCount: 0,
        negativeCount: 0,
      }
    }
  ...
  const displayedArticles = showAll
    ? sortedArticles
    : balancedArticles.slice(0, displayCount)
    ...
    <Card className={isMarketNews ? 'p-6 shadow-lg' : 'border-border'}>
      ...
      {isExpanded && (
        <CardContent className={isMarketNews ? 'p-0' : 'space-y-4'}>
          ...
          {symbol &&
            newsIntelligence &&
            newsIntelligence.keyEvents &&
            newsIntelligence.keyEvents.length > 0 && (
              <div>
          ...
          {articles.length === 0 ? (
            ...
            <>
              ...
              {!showAll && sortBy !== 'recent' && (
                <p className="text-xs text-text-muted mb-3">
                  Showing {displayedArticles.length} of {sortedArticles.length}{' '}
                  articles
                </p>
              )}
              <div className="space-y-2">
