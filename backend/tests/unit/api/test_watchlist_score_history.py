...
class TestGetScoreHistoryEndpoint:
    ...
    async def test_returns_score_history_for_valid_item(
        self, mock_storage: MagicMock, sample_item_data: dict, sample_snapshots_data: list[dict]
    ) -> None:
        ...
        mock_storage.query.side_effect = [item_df, snapshots_df]

        with patch("app.api.watchlist.storage", mock_storage):
    ...
    async def test_returns_empty_history_when_no_snapshots(
        self, mock_storage: MagicMock, sample_item_data: dict
    ) -> None:
    ...
    async def test_respects_days_parameter(
        self, mock_storage: MagicMock, sample_snapshots_data: list[dict]
    ) -> None:
    ...
    @pytest.mark.asyncio
    async def test_handles_null_raw_metrics(self, mock_storage: MagicMock) -> None:
    ...
    @pytest.mark.asyncio
    async def test_raises_500_on_database_error(self, mock_storage: MagicMock) -> None:
    ...
    @pytest.mark.asyncio
    async def test_uses_watchlist_snapshots_v_view(self, mock_storage: MagicMock) -> None:
        """Test that the endpoint queries watchlist_snapshots_v view."""
        item_df = MagicMock()
        item_df.is_empty.return_value = False
        item_df.to_dicts.return_value = [{"symbol": "AAPL"}]
        ...
        with patch("app.api.watchlist.storage", mock_storage):
            ...
            # Check second query call (snapshots query)
            snapshots_query_call = mock_storage.query.call_args_list[1]
            query_sql = snapshots_query_call[0][0]

            assert "watchlist_snapshots_v" in query_sql
            assert "ORDER BY fetched_at DESC" in query_sql
