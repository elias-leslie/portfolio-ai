    ...
    *,
    ...
    for group in _merchant_groups(rows):
        ...
        streams.append(
            IncomeActualsStream(
                label=label,
                owner=_detect_owner(group, owner_names or []),
                cadence=cadence,
                ...
                active=active,
                portfolio_yield=_is_portfolio_yield(label),
            )
        )
    streams.sort(key=lambda s: s.monthly_average, reverse=True)

    total_income = sum(float(row["amount"]) for row in rows)
    active_income = sum(
        s.monthly_average for s in streams if s.active and not s.portfolio_yield
    )
    return IncomeActuals(
        generated_at=generated_at,
...
class RetirementIncomeActualsService:
    ...
    def build(self) -> IncomeActuals:
        # Deferred so importing this module doesn't pull the storage stack.
