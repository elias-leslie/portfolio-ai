...
def row_to_profile(
    ...
    *,
    ...
    return HouseholdProfile(
        ...
        target_retirement_spend=to_float(row[9]),
        retirement_inflation_rate=to_float(row[10]),
        retirement_horizon_years=to_int(row[11]),
        primary_social_security_monthly=to_float(row[12]),
        spouse_social_security_monthly=to_float(row[13]),
        primary_social_security_annual_earnings=to_float(row[14]),
        spouse_social_security_annual_earnings=to_float(row[15]),
        primary_social_security_start_age=to_int(row[16]),
        spouse_social_security_start_age=to_int(row[17]),
        social_security_payable_ratio=to_float(row[18]),
        filing_status=str(row[19]) if row[19] is not None else None,
        state_of_residence=str(row[20]) if row[20] is not None else None,
        effective_tax_rate=to_float(row[21]),
        marginal_federal_tax_rate=to_float(row[22]),
        marginal_state_tax_rate=to_float(row[23]),
        emergency_fund_target_months=to_float(row[24]),
        emergency_fund_target_amount=to_float(row[25]),
        notes=str(row[26]) if row[26] is not None else None,
        created_at=iso(row[27]),
        updated_at=iso(row[28]),
    )
