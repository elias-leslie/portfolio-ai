...
export interface HandlerContext {
  ...
  setCurrentResponse: (updater: ContentBlock[] | ((prev: ContentBlock[]) => ContentBlock[])) => void;
...
export function handleStreamMessage(msg: WebSocketMessage, ctx: HandlerContext): void {
  if (msg.data) {
...
export function handleAgentStart(msg: WebSocketMessage, ctx: HandlerContext): void {
  if (msg.agent) {
...
export function handleAgentDone(msg: WebSocketMessage, ctx: HandlerContext): void {
...
export function handleDiscussionRound(msg: WebSocketMessage, ctx: HandlerContext): void {
  if (msg.round) {
    ctx.setMessages(prev => [
      ...prev,
      {
...
export function handleDone(ctx: HandlerContext): void {
  const blocks = ctx.currentResponseRef.current;
  if (blocks.length > 0) {
    const agent = ctx.currentRespondingAgent
...
export function handlePermissionRequest(msg: WebSocketMessage, ctx: HandlerContext): void {
...
export function handleProviderConfirmation(msg: WebSocketMessage, ctx: HandlerContext): void {
  if (msg.name) {
