...
export function ImportCenterSidebar({
  ...
  return (
    <div className="space-y-3">
      {importCenter ? (
        <IntakeCoverageCards importCenter={importCenter} />
      ) : null}
      {evidenceAccounts.length > 0 ? (
        <EvidenceAccountsCard accounts={evidenceAccounts} />
      ) : null}
      {importCenter?.automations.length ? (
        <AutomationsCard automations={importCenter.automations} />
      ) : null}
      {importCenter?.supportedDocuments.length ? (
        <SupportedDocumentsCard
          supportedDocuments={importCenter.supportedDocuments}
        />
      ) : null}
...
function EvidenceAccountsCard({
  accounts,
}: {
  accounts: HouseholdEvidenceAccount[]
}) {
  return (
    <div className="rounded-2xl border border-border/40 bg-surface-muted/20 p-4">
      ...
      <p className="mt-1 text-sm text-text-muted">
        Evidence-derived account snapshots that can immediately inform the money
        system.
      </p>
      <div className="mt-3 space-y-3">
        {accounts.slice(0, 5).map((account) => {
          ...
          return (
            <div
              key={account.id}
              className="rounded-2xl border border-border/40 bg-surface/60 p-3"
            >
      ...
      <div className="rounded-2xl border border-border/40 bg-surface-muted/20 p-4">
        ...
        {importCenter.suggestedFirstUploads.length > 0 ? (
          ...
          <p className="mt-2 text-sm text-text-muted">
            Intake guidance appears after Jenny sees gaps in the household
            record.
          </p>
        )}
      </div>
...
function SupportedDocumentsCard({
  supportedDocuments,
}: {
  supportedDocuments: ImportCenter['supportedDocuments']
}) {
