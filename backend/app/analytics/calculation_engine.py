...
def calculate_portfolio_sharpe(
    ...
    """Publish a ratio only when valuations and external flows support it."""
    return performance_report(storage, account_ids, risk_free_rate)["sharpe_ratio"]


def _get_portfolio_daily_returns(storage: PortfolioStorage, account_ids: list[str]) -> list[float]:
    """Compatibility reader; use the validated performance report for display."""
    report = performance_report(storage, account_ids)
    return report.get("daily_returns", [])
