...
import type { DayBarFreshnessInfo } from '@/lib/api/status'
...
export function DataFreshnessCard({ freshness }: DataFreshnessCardProps) {
      ...
      <Card className="border-border">
        ...
        <CardContent>
          <p className="text-sm text-muted-foreground">
            No dayBars data available. Price data may not have been ingested
            yet.
          </p>
        </CardContent>
      </Card>
  ...
  const stale = freshness.filter(
    (f) => (f.ageDays ?? 999) > 1 && (f.ageDays ?? 999) <= 7,
  )
  ...
  const getStatusBadge = (ageDays?: number) => {
    ...
    if (ageDays <= 1) {
      return (
        <Badge className="bg-status-success text-text-inverted">Fresh</Badge>
      )
    }
    if (ageDays <= 7) {
      return (
        <Badge className="bg-status-warning text-text-inverted">Stale</Badge>
      )
    }
    ...
    <Card className="border-border">
      <CardHeader>
        <CardTitle className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <Badge className="bg-status-success text-text-inverted">
              {fresh.length} Fresh
            </Badge>
            {stale.length > 0 && (
              <Badge className="bg-status-warning text-text-inverted">
                {stale.length} Stale
              </Badge>
            )}
            {veryStale.length > 0 && (
      ...
      <CardContent>
        ...
        <div className="mt-4 p-3 bg-muted/50 rounded-lg text-xs text-muted-foreground">
          ...
          <p>
            Data freshness indicates how recent the last dayBars entry is for
            each symbol. Fresh data (&lt;1 day) is normal. Stale data may
            indicate ingestion issues or market holidays.
          </p>
        </div>
      </CardContent>
    </Card>
