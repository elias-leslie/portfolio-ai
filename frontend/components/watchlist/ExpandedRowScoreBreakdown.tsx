...
import type { WatchlistItem, ScoreComponent } from "@/lib/api/watchlist";
...
const PILLAR_CONFIG = {
    price: {
        ...
        icon: "💰",
        defaultWeight: 22,
        tooltip: "Price momentum based on recent price changes, volatility (beta), and relative volume. Higher scores indicate positive momentum.",
    },
    technical: {
        label: "Technical",
        icon: "📊",
        defaultWeight: 22,
        tooltip: "Technical indicator analysis including RSI, MACD, trend strength, and Bollinger Bands. Higher scores indicate bullish technical setup.",
    },
    fundamental: {
        label: "Fundamental",
        icon: "🏢",
        defaultWeight: 26,
        tooltip: "Financial health score combining valuation (P/E, PEG), growth metrics, balance sheet health, and analyst sentiment.",
    },
    catalyst: {
        label: "Catalyst",
        icon: "📰",
    ...
    performance_factor: {
        ...
        icon: "🎯",
        defaultWeight: 5,
        tooltip: "Active strategy performance based on 30-day Sharpe ratio. Only shown when an active strategy exists.",
    },
} as const;
            ...
            <AccordionTrigger className="hover:no-underline px-3 py-2">
                <div className="flex items-center gap-2 flex-1">
                    ...
                    <Tooltip>
                        ...
                        <TooltipContent side="top">
                            <p className="text-xs">Weight: {weightPercent}% of overall score</p>
                        </TooltipContent>
                    </Tooltip>
                    {pillar.stale && (
                        ...
                        <div className="space-y-2">
                            ...
                            <div className="grid grid-cols-2 sm:grid-cols-3 gap-x-4 gap-y-1 text-xs">
                                {Object.entries(pillar.metadata!)
                                    .filter(([key]) => !["source", "cached_at", "formula"].includes(key))
                                    .map(([key, value]) => (
