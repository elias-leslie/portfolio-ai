...
def _insert_metrics(storage: PortfolioStorage, date_str: str, signal_counts: dict[str, Any], avg_scores: dict[str, Any],
    ...
    with storage.connection() as conn:
        conn.execute(
            ...
            "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
            [str(uuid.uuid4()), date_str, "daily", sum(signal_counts.values()), signal_counts["BUY"],
             signal_counts["HOLD"], signal_counts["AVOID"], trades_stats["total"], trades_stats["wins"],
             trades_stats["losses"], win_rate, trades_stats["avg_return"], trades_stats["best"],
             trades_stats["worst"], cumulative_return, avg_scores["overall"], avg_scores["technical"],
             avg_scores["fundamental"], score_stdev, reviews_stats["total"], reviews_stats["disagreements"],
             disagreement_rate, reviews_stats["provider_disagreements"], provider_disagreement_rate,
             reviews_stats["avg_agreement"], reviews_stats["major_disagreements"],
             reviews_stats["minor_disagreements"], datetime.now(UTC)],
        )
        conn.commit()
...
def collect_daily_strategy_metrics() -> dict[str, object]:
    ...
    try:
        ...
        reviews_stats, disagreement_rate, provider_disagreement_rate = _get_review_metrics(storage, yesterday)
        cumulative_return = _get_cumulative_return(storage, yesterday)
        _insert_metrics(storage, yesterday, signal_counts, avg_scores, score_stdev, trades_stats,
                       win_rate, reviews_stats, disagreement_rate, provider_disagreement_rate, cumulative_return)
        logger.info(f"Strategy metrics collected for {yesterday}", extra={
            "date": yesterday, "signals": sum(signal_counts.values()), "win_rate": win_rate,
            "disagreement_rate": disagreement_rate, "provider_disagreement_rate": provider_disagreement_rate,
        })
        return {"status": "success", "date": yesterday, "metrics": {
            "signals": signal_counts, "trades": trades_stats, "reviews": reviews_stats,
            "win_rate_pct": win_rate, "disagreement_rate_pct": disagreement_rate,
