...
export function TaskResultDisplay({
  ...
  const summaryEntries = Object.entries(result).filter(
    ([k, v]) =>
      !skipFields.includes(k) && !detailFields.includes(k) && !Array.isArray(v),
  )
  const detailEntry = Object.entries(result).find(
    ([k, v]) =>
      detailFields.includes(k) &&
      Array.isArray(v) &&
                ...
                {isObjectArray && columns ? (
                  <table className="w-full text-xs">
                    <thead className="sticky top-0 bg-background border-b">
                      <tr>
                        {columns.map((col) => (
                          <th
                            key={col}
                            className="text-left p-2 font-medium capitalize whitespace-nowrap"
                          >
                            {col.replace(/_/g, ' ')}
                          </th>
                        ))}
                    ...
                    <tbody>
                      {arr.map((item, i) => (
                        <tr
                          key={i}
                          className="border-b border-border/20 last:border-0 hover:bg-surface-muted/20"
                        >
                          {columns.map((col) => (
                            <td
                              key={col}
                  ...
                  <div className="p-2 space-y-1">
                    {arr.map((item, i) => (
                      <div
                        key={i}
                        className="text-xs font-mono text-muted-foreground py-1 border-b border-border/20 last:border-0"
                      >
                        {String(item)}
                      </div>
                    ))}
      ...
      <DialogContent className="!max-w-[90vw] !w-[1400px] !h-[85vh] flex flex-col overflow-hidden">
        <DialogHeader className="flex-shrink-0">
          ...
          <DialogDescription>
            ...
            {' • '}
            {results.length} tasks • {successCount} success • {errorCount}{' '}
            errors
          </DialogDescription>
        </DialogHeader>
              ...
              >
                <summary className="flex items-center gap-2 p-3 cursor-pointer select-none hover:bg-surface-muted/20">
                  ...
                  <Badge
                    ...
                    className={
                      result.status === 'success' ? 'bg-status-success' : ''
                    }
                  >
                    {result.status}
                  </Badge>
                  {result.error && (
                    <span className="text-status-error text-xs ml-2 truncate">
