...
def _fields_with_confident_inferences(
...
def _found_items(
    executive: Any, resolved_values: list[HouseholdResolvedValue]
) -> list[str]:
    found: list[str] = []
    if executive.recurring_merchant_count > 0:
        s = "s" if executive.recurring_merchant_count != 1 else ""
        found.append(
            f"Detected {executive.recurring_merchant_count} recurring monthly commitment{s} from your statements"
        )
    if executive.average_monthly_essentials > 0:
        ...
        found.append(
            ...
            f"across {executive.coverage_months} month{s}"
        )
    if executive.tracked_expense_count > 0:
        s = "s" if executive.tracked_expense_count != 1 else ""
        found.append(
            f"{executive.tracked_expense_count} transaction{s} tracked and categorized from your statements"
        )
    inferred_count = sum(
        1 for rv in resolved_values if rv.source == "jenny_inference" and rv.value is not None
    )
    if inferred_count > 0:
...
def _build_portfolio_context(
    *,
    ...
    annual_spend: float | None = (
        (monthly_essential + (monthly_discretionary or 0)) * 12
    ...
    *,
    ...
    overview = HouseholdOverview(
        ...
        next_best_action=service._next_best_action(
            documents, visibility_score, questions=questions, resolved_values=resolved_values,
        ),
    )
...
def _build_jenny_brief(
    profile: Any, reports: Any, resolved_values: list[HouseholdResolvedValue]
) -> JennyMoneyBrief:
...
def _assemble_finance_dashboard(
    *,
    ...
    return HouseholdFinanceDashboard(
        ...
        jenny_brief=_build_jenny_brief(profile, reports, resolved_values),
