...
def _prime_hatchet_sdk_env() -> None:
    """Expose repo-local Hatchet settings through the SDK's env contract."""
...
def _install_hatchet_shutdown_404_guard() -> None:
    ...
    During SIGTERM shutdown races, Hatchet can evict the listener's worker
    record before ``pause_task_assignment()`` reaches the REST update. That 404
    is effectively "already paused / already gone" and should not bubble out as
