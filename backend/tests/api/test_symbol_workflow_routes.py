...
def test_get_symbol_workflow_returns_service_payload(mocker) -> None:
    mocker.patch(
        "app.api.symbols.router._workflow_service",
        return_value=mocker.Mock(
            get_workflow=mocker.Mock(
                return_value={
                    "symbol": "VTI",
                    "stage": "review_due",
...
def test_transition_symbol_workflow_returns_updated_state(mocker) -> None:
    ...
    mocker.patch("app.api.symbols.router._workflow_service", return_value=service)
    mocked = service.transition
...
def test_get_home_automation_center_returns_guardrails_and_runs(mocker) -> None:
    mocker.patch(
        "app.api.home._automation_center_service",
        return_value=mocker.Mock(
            get_center=mocker.Mock(
                return_value={
