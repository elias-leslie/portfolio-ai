...
export const strategyKeys = {
  ...
  detail: (strategyId: string) =>
    [...strategyKeys.details(), strategyId] as const,
...
export function useGenerateStrategy() {
  ...
  return useMutation({
    ...
    onMutate: ({ symbol }) => {
      toast.loading(`Generating strategy for ${symbol}...`, {
        id: `generate-${symbol}`,
      })
    },
    ...
    onSuccess: (data, { symbol }) => {
      ...
      if (data.status === 'completed') {
        ...
        toast.error(
          data.errorMessage || `Strategy generation failed for ${symbol}`,
        )
      }
...
export function useGenerateStrategiesBatch() {
  ...
  return useMutation({
    mutationFn: (request: GenerateBatchRequest) =>
      generateStrategiesBatch(request),
    ...
    onSuccess: (data) => {
      ...
      if (generated > 0) {
        toast.success(
          `Generated ${generated} strategies (${processed} symbols processed)`,
        )
      } else {
...
export function useStrategySeeds(params?: {
  status?: string
  symbol?: string
  limit?: number
}) {
