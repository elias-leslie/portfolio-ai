    ...
    *,
    ...
    prompt_parts.extend(
        [
            ...
            "Transcribe receipt lines in printed order and do not merge, reorder or summarise them. Two adjacent lines with the same item code and price are two separate purchases of that item: emit both, never collapse them into one row.",
            "Warehouse clubs print a markdown on its own line below the item it reduces, with the amount ending in a minus sign (for example '0000388263 / 1761722    4.50-'). Never drop those lines and never put them in line_items. Report each one in transactions[].discounts as {applies_to: <description of the item it reduces>, amount: <positive number>}, and leave that item's own amount at the printed shelf price. Do not do the subtraction yourself.",
