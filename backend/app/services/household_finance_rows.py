...
def row_to_profile(
    ...
    *,
    ...
    return HouseholdProfile(
        ...
        filing_status=str(row[10]) if row[10] is not None else None,
        state_of_residence=str(row[11]) if row[11] is not None else None,
        effective_tax_rate=to_float(row[12]),
        marginal_federal_tax_rate=to_float(row[13]),
        marginal_state_tax_rate=to_float(row[14]),
        emergency_fund_target_months=to_float(row[15]),
        emergency_fund_target_amount=to_float(row[16]),
        notes=str(row[17]) if row[17] is not None else None,
        created_at=iso(row[18]),
        updated_at=iso(row[19]),
    )
...
def row_to_tracked_account(
    ...
    *,
    ...
    return HouseholdTrackedAccount(
        ...
        match_key=str(row[5]) if row[5] is not None else None,
