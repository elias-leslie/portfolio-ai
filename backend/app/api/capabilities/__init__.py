...
- Capability insights (capability_insights) - analysis findings
...
- insights_router: Insight CRUD endpoints (create, list, review)
...
# NOTE: features_router and insights_router MUST come before capabilities_router
# because capabilities_router has a /{type}/{id} catch-all route that would
# otherwise intercept /features/* and /insights/* requests
router = APIRouter(prefix="/api/capabilities", tags=["capabilities"])
router.include_router(features_router)  # Must be first (has /features/* routes)
router.include_router(insights_router)  # Before catch-all (has /insights/* routes)
