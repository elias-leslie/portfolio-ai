...
function calculateMonthlyReturns(equityCurve: EquityPoint[]): MonthlyReturn[] {
  ...
  equityCurve.forEach((point) => {
    ...
    const equity =
      typeof point.equity === 'string' ? parseFloat(point.equity) : point.equity
  ...
  return (
    ...
    >
          ...
          <thead>
            <tr>
              ...
              {MONTH_NAMES.map((month) => (
                <th
                  key={month}
                  className="px-2 py-1 text-center text-text-muted font-medium"
                >
                  {month}
                </th>
              ))}
              <th className="px-2 py-1 text-center text-text-muted font-medium">
                Total
              </th>
            </tr>
          </thead>
      ...
      <p className="mt-4 text-xs text-text-muted text-center">
        Green = positive returns • Red = negative returns • Darker = larger
        magnitude
      </p>
    </SectionCard>
  )
