...
export interface UseWebSocketConnectionOptions {
  ...
  setCurrentRespondingAgent: Dispatch<
    SetStateAction<'claude' | 'gemini' | null>
  >
...
export function useWebSocketConnection({
  ...
  const connect = useCallback(() => {
    ...
    const orderParam =
      agentProvider === 'both'
        ? `&order=${roundtableOrder}&max_turns=${maxTurns}`
        : ''
    const ws = new WebSocket(
      `${wsUrl}/ws/${currentSessionId}?provider=${providerParam}${orderParam}`,
    )
    ...
    ws.onclose = () => {
      ...
      if (thisGeneration === connectionGenRef.current) {
        ...
        console.log(
          'Ignoring stale onclose for generation:',
          thisGeneration,
          'current:',
